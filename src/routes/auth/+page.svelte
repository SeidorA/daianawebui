<script lang="ts">
	...
	import { isSafeReturnTo, parseAllowedReturnToOrigins, parseHandoffUrl } from '$lib/utils/handoff';
	...
	const signInHandler = async () => {
		const sessionUser = await userSignIn(
			($config?.features.auth_trusted_header ?? false) ? '' : email,
			($config?.features.auth_trusted_header ?? false) ? '' : password
		).catch((error) => {
			toast.error(`${error}`);
			return null;
		});
	...
	onMount(async () => {
		...
		if (handoffToken) {
			...
			if (signedIn) {
				if (returnTo && isSafeReturnTo(returnTo, window.location.origin, allowedReturnToOrigins)) {
